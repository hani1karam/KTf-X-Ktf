...
class LoanVC: BaseViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    func loadproblem(){
        ...
        Reguest.sendRequest(method: .get, url: showproblem_Loan, completion:
    ...
    @IBAction func AddPost(_ sender: Any) {
        let Forget = LoanPostProblemVC.instance()
            ...
            if error  == nil{
                DispatchQueue.main.async {
                    guard let status = user?
                        .status else{return}
                    if status == true{
                        self.showToast(message:  "تم اضافه اللايك بنجاح")
                        self.HideLoading()
                        
                    }
                    else{
                        self.showToast(message:"قمت بذلك مسبقا")
                        self.HideLoading()
